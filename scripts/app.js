// scripts/app.js — compact, ownership filter + legend
(async function () {
  ...
  if (owners.includes("Gateway")) ownerColor["Gateway"] = "#ff007a"; // highlight
  ...
  legend.innerHTML = owners.slice(0, 10).map(o =>
    `<span style="display:inline-flex;align-items:center;margin-right:10px;font-size:12px;">
       <span style="width:10px;height:10px;border-radius:50%;background:${ownerColor[o]};display:inline-block;margin-right:6px;border:1px solid #334155"></span>${o}
     </span>`
  ).join("") + (owners.length > 10 ? `<span style="font-size:12px;">+${owners.length-10} more</span>` : "");

  function markerStyle(owner) {
    const s = { radius: 6, weight: 1, color: '#334155', fillColor: ownerColor[owner] || '#64748b', fillOpacity: 0.9, opacity: 0.9 };
